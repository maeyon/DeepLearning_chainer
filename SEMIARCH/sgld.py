...
class SA(chainer.Chain):
    def __init__(self):
        super(SA, self).__init__(l1 = L.Linear(None, 200),
                                 l2 = L.Linear(None, 200),
...
class SGLD(chainer.optimizer.GradientMethod):
    ...
    def update_one_cpu(self, param, state):
        g = param.grad
        param.data -= 0.5 * self.lr * (8. * g + self.weight * param.data) + np.random.normal(0, self.lr, g.shape)
...
trainer = training.Trainer(updater, (50, 'epoch'), out='SGLD')
