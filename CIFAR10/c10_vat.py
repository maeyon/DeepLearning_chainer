...
N_train_labeled = 100
N_train_unlabeled = 50000
N_test = 10000
train_l, train_ul, test_set = load_cifar(scale=1.0/128.0, shift=-1.0, N_train_labeled=N_train_labeled, N_train_unlabeled=N_train_unlabeled, N_test=N_test)
...
class Encoder(chainer.Chain):
    ...
    def __call__(self, x, test=False):
        h = self.bn1(F.relu(self.conv1(x)), test=test)
        h = F.max_pooling_2d(h, 2)
        h = self.bn2(F.relu(self.conv2(x)), test=test)
        h = F.max_pooling_2d(h, 2)
...
for epoch in range(len(alpha_plan)):
    ...
    sum_loss_l = 0
    sum_loss_ul = 0
    for it in range(50000/batchsize_ul):
    ...
    print "classification loss, vat loss: ", sum_loss_l/(50000/batchsize_ul), sum_loss_ul/(50000/batchsize_ul)
